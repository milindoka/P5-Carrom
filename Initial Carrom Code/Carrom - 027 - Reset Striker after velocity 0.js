...
function draw() 
{
  ...
  fill('#bfe693');
  rect(0, 0, carromwidth, carromheight,32); // 30 is the corner radius
...
class Striker extends Mover {
  ...
  setVelocity() {
    if (!this.dragStart) {
      this.dragStart = createVector(mouseX - 50, mouseY - 50);
    }
  ...
  show() {
    super.show();
    if (mouseIsPressed && !this.isLaunched) {
      stroke(255, 0, 0);
      strokeWeight(2);
